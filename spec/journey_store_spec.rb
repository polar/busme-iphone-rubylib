...
describe Platform::JourneyStore do
  let(:route_spec) {
    "
      ...
      se_lon='-76.118363'
      se_lat='42.995355'
      patternids='b2d03c4880f6d57b3b4edfa5aa9c9211'
      >
      </Route>
      "
  }
  let(:journey_spec) {
    ...
    <Route curloc='http://busme-apis.herokuapp.com/masters/50fcc339223520000c000088/apis/1/968f501b3e02890cffa2a1e1b80bc3ca'
        ...
        routeid='9864eb9e615f740526e93f6297e29435'
        >
        <Links>
  ...
  let(:route) {
    doc = REXML::Document.new(route_spec)
  ...
  let(:journey) {
    doc = REXML::Document.new(journey_spec)
    tag = Api::Tag.new(doc.root)
    route = Api::Route.new
    route.loadParsedXML(tag)
    route
  ...
  let(:pattern) {
    doc = REXML::Document.new(pattern_spec)
    tag = Api::Tag.new(doc.root)
